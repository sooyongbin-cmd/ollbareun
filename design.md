...
**`operational-list-search-section`** — Used on manager list pages before any tabular result set. Background `{colors.canvas-parchment}`, 1px solid `{colors.hairline}` border, rounded `{rounded.lg}` (18px), padding `{spacing.xl}` (32px). Contains only query/filter controls and primary creation actions, such as the employee name search input plus "직원 등록". Do not place result counts, loading states, empty states, or tables in this section. Give the section an accessible label that names the task, such as `aria-label="직원 검색"`.

**`operational-list-results-section`** — Used immediately after `{component.operational-list-search-section}` on manager list pages. Same card shell: `{colors.canvas-parchment}`, 1px solid `{colors.hairline}` border, rounded `{rounded.lg}` (18px), padding `{spacing.xl}` (32px). Contains the list state only: total count, filtered result count, loading text, error text, empty state, and the table. Give the section an accessible label that names the data surface, such as `aria-label="직원 목록"`. The table wrapper may use `{colors.canvas}` with `{rounded.lg}`/16px and a hairline border, but it must not become a nested decorative card; it is a functional table frame.

**Operational list rule** — Search/filter controls and results must be separate sibling sections. The user should be able to scan "how to query" and "what was returned" as two distinct surfaces. This applies to employee rosters, worksites, assignments, attendance lists, and report lists. Keep the outer page rhythm at `{spacing.lg}` (24px) between the header, search section, and results section. Avoid putting a search toolbar, counters, and table in one all-purpose card.
