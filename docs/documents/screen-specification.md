...
| 화면 ID | 화면명 | 사용자 | 주요 기능 |
| --- | --- | --- | --- |
| WEB-001 | 회사소개 홈페이지 | 전체 | 기업·서비스·사회적 가치 소개, 상담 및 서비스 이동 |
| WEB-003 | 우선구매 혜택 안내 모달 | 전체 | 사회적기업 우선구매 혜택 안내 |
...
| COM-002 | 오프라인 안내 | 전체 | 네트워크 연결 안내 및 재시도 |
| M-AUTH-001 | 관리자 로그인 | 관리자 | 구글로그인, 최초 관리자 등록 |
| G-AUTH-001 | 근무자 로그인 | 근무자 | 이름·연락처 인증 |
...
- 주요 구성:
  - 올바름 로고와 `상담 신청하기` 버튼
  - 기업 핵심 메시지와 대표 이미지
  - 체계적인 전문 교육, 철저한 현장 관리, 안정적인 서비스 품질 소개 카드
  - 사회적기업 우선구매 제도 안내 영역
  - 취약계층 고용률, 고객 만족도, 지역사회 공헌 지표
  - 회사 연락처, 주소, 고객지원 정보
  - 관리자 및 현장 근무자 서비스 바로가기
- 동작:
  - `상담 신청하기` 선택 → 상담 신청 화면 표시
  - `혜택 자세히 보기` 선택 → `WEB-003`
  - 관리자 바로가기 → `M-AUTH-001` 또는 인증된 경우 `M-001`
  - 현장 근무자 바로가기 → `G-AUTH-001`
- 반응형: 서비스와 지표 카드는 모바일에서 1열, 넓은 화면에서 3열로 배치한다.
...
- 주요 내용:
  - 공공기관의 사회적기업 제품·서비스 우선구매 취지
  - 일자리 창출과 취약계층 지원 등 사회적 가치 실현
  - 체계적인 전문 인력을 통한 시설관리 서비스 제공
- 버튼: 확인, 닫기
- 동작: 확인 또는 닫기 선택 시 모달을 닫고 `WEB-001`을 유지한다.
...
- 인증 방식: 구글로그인
- 버튼: 구글로그인, 최초 관리자 등록, 이전 화면
- 처리:
  - 구글 계정 인증이 완료되면 등록된 관리자 계정인지 확인한다.
  - 인증 완료 후 요청했던 관리자 화면 또는 대시보드로 이동한다.
- 최초 관리자 영역:
  - 관리자 계정이 없을 때만 표시한다.
  - 구글 계정 인증과 최초 관리자 등록 코드를 확인한다.
...
`회사소개 홈페이지 → 우선구매 혜택 확인 → 회사소개 홈페이지 → 관리자/현장 근무자 서비스 선택`
