...
describe("POST /api/guard/special-remarks/report", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("creates a special remark report", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/guard/special-remarks/report", {
        method: "POST",
    ...
    expect(createSpecialRemarkReport).toHaveBeenCalledWith(body);
  });
});
