...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ report: await createSpecialRemarkReport({ ...body, employeeId: employee.id, employeeName: employee.name, worksiteId: worksite.id, worksiteName: worksite.name }) });
  } catch (error) {
