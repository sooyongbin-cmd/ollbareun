...
describe("Figma homepage mobile responsive layout", () => {
  ...
  it("uses the eight discrete Figma states for the landing header", () => {
    ...
    expect(headerRules).toMatch(
      /\.publicSite \.header:not\(\.mobileHeaderOpen\) \.headerInner\s*\{\s*width: 100%;[\s\S]*?padding-right: var\(--landing-header-padding-right\);[\s\S]*?padding-left: var\(--landing-header-padding-left\);/,
    );
    expect(headerRules).not.toMatch(/width: (?:962|924)px;/);
    ...
    expect(headerRules).toMatch(
      /@media \(max-width: 360px\)[\s\S]*?--landing-header-padding-left: 18px;[\s\S]*?--landing-header-padding-right: 18px;/,
    );
    expect(headerRules).toMatch(
