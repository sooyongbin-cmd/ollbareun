    ...
    <section className="space-y-[1.5rem]">
      <header>
        <div className="space-y-3">
          <h1 className="text-[1.75rem] leading-[1.2]">로그현황</h1>
      ...
      <section aria-label="로그 검색" className="bg-muted/40 rounded-xl p-[2rem] border border-border/50">
        <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_7.5rem_10rem] md:items-end">
          <div className="space-y-2">
            <label className="text-[0.875rem] font-semibold text-muted-foreground ml-1" htmlFor="log-guard-name">
              경비원 이름
            </label>
            <Input
              className="w-full"
          ...
          <label className="flex h-[3rem] items-center gap-2 text-[0.9375rem] font-semibold text-foreground/80">
            <Checkbox
          ...
          <div className="space-y-2">
            <label className="text-[0.875rem] font-semibold text-muted-foreground ml-1" htmlFor="log-push-status">
              Push 상태
            </label>
            <NativeSelect
              className="w-full"
                ...
                {logs.length === 0 ? (
                  ...
                  logs.map((log) => (
                    <TableRow key={log.id} className="hover:bg-muted/40 transition-colors">
                      ...
                      <TableCell data-label="로그아웃 Push 결과" className="min-w-[13.75rem] text-muted-foreground">{getLogoutPushSummary(log)}</TableCell>
                      <TableCell data-label="오류/상세" className="min-w-[11.25rem] text-muted-foreground">{getDetailSummary(log)}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
