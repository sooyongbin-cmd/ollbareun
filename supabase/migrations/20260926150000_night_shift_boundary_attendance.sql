...
begin
  ...
  loop
    ...
    if style = '2' then
      ...
      -- Start with an in-time only, end with an out-time only, and keep both
      -- times on intermediate dates. A one-day assignment represents one full
      -- night shift, so it keeps both values on the same row.
